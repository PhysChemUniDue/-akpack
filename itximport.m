function varargout = itximport(filename, varargin)
% ITXIMPORT imports the waves from an *.itx file (IGOR Text File).
%
%   ITXIMPORT() opens an open file dialog box where you can select the
%   itx file and imports the waves with their wavenames as they are defined
%   in the itx file into the workspace.
%   ITXIMPORT(FILENAME) imports the itx file FILENAME into the workspace.
%   ITXIMPORT(FILENAME, OPTION) imports the itx file FILENAME with OPTION.
%   Available options are:
%
%   'base'      -   import into workspace
%   'struct'    -   import into a structure S
%                   Example: S = ITXIMPORT(FILENAME, 'struct')
